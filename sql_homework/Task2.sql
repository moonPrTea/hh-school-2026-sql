...
WITH test_data(id, id_profession, id_candidate, id_employment_type, id_work_format, compensation_from, working_hours, new_date) AS (
...
INSERT INTO resumes(id_position, id_candidate, id_employment_type, id_work_format, compensation_from, working_hours, created_date)
SELECT  id_profession,id_candidate, id_employment_type, id_work_format, compensation_from, working_hours, new_date FROM test_data;


WITH test_data(id, title, salary, work_hours, id_area, id_company, id_work_format, id_profession, created_date) AS (
    SELECT generate_series(1, 10000) AS id,
           md5(random()::varchar) AS title,
           (random() * 100000)::int AS salary,
           (random() * 8 + 1)::int AS work_hours,
...
INSERT INTO vacancies(title, compensation_from, compensation_to, work_hours, id_area, id_company, id_work_format, id_position, created_date)
SELECT title, salary, salary + 10000, work_hours, id_area, id_company, id_work_format, id_profession, created_date FROM test_data;
...
WITH test_data(id, id_resume, id_vacancy, created_date) AS (
    SELECT generate_series(1, 200000) AS id,
           ...
           (random() * 199 + 1)::int AS id_vacancy,
           (current_date - (random() * 3650)::int) AS created_date
)
...
WITH test_data(id, id_vacancy, id_skill) AS (SELECT generate_series(1, 3) AS id,
...
WITH test_data(id, id_resume, id_skill) AS (SELECT generate_series(1, 3) AS id,
                                          (random() * 99999 + 1)::int as id_resume,
                                          (random() * 199 + 1)::int  as id_skill
)
